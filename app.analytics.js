...
(function () {
  ...
  function lastMetrics() {
    ...
    return state.metrics.slice().sort((a, b) => b.date.localeCompare(a.date))[0];
  }
  ...
  function build1RMSeries() {
    ...
    Object.values(byLift).forEach((arr) => arr.sort((a, b) => a.x.localeCompare(b.x)));
    return byLift;
  }
  ...
  function render1RMChart() {
    ...
    const datasets = Object.keys(byLift).slice(0, 5).map((lift) => ({
      label: lift,
      data: byLift[lift],
      parsing: false
    }));
  ...
  function buildBodySeries() {
    ...
    const weight = [];
    const bf = [];
    const list = state.metrics.slice().sort((a, b) => a.date.localeCompare(b.date));
    list.forEach((m) => {
      labels.push(m.date);
  ...
  function calcProjection(days = 60) {
    const s = state.settings;
    const m = lastMetrics();
    const startWeight = (parseNum($('#caloriesInput')?.value) && latestWeightLb()) || latestWeightLb();
    ...
    const mult = (() => {
      ...
      return { sedentary: 1.2, light: 1.375, moderate: 1.55, very: 1.725, athlete: 1.9 }[lvl] || 1.55;
    })();
  ...
  function renderProjection() {
    ...
    if (!proj) {
      box.textContent = 'Add weight in Body Metrics (or Settings) to estimate projections.';
      return;
    }
    box.innerHTML = `Projected trend based on <strong>${$('#caloriesInput')?.value || Math.round(proj.tdee)} kcal/day</strong>. Estimated TDEE: <strong>${Math.round(proj.tdee)} kcal</strong>.`;

    makeChart(ctx, {
      type: 'line',
      data: { labels: proj.labels, datasets: [{ label: 'Projected Weight (lb)', data: proj.series }] },
  ...
  function renderMiniGoals() {
    ...
    if (g.weight && g.targetDate && weightNow != null) {
      ...
      const pct = deltaTotal === 0 ? 100 : Math.max(0, Math.min(100, (deltaNow / deltaTotal) * 100));
      const daysLeft = Math.ceil((new Date(g.targetDate) - new Date()) / 86400000);
      rows.push(`
        <div><strong>Body Weight:</strong> ${fmt(weightNow)} lb → ${fmt(g.weight)} lb</div>
        <div class="muted">Progress: ${isFinite(pct) ? pct.toFixed(0) : 0}% • ${daysLeft >= 0 ? daysLeft : 0} days left</div>
    ...
    if (g.liftName && g.lift1RM) {
      ...
      rows.push(`
        <div><strong>${g.liftName} 1RM:</strong> ${fmt(best)} lb → ${fmt(g.lift1RM)} lb</div>
        <div style="height:8px; background:#1f2026; border-radius:999px; overflow:hidden; margin:.35rem 0">
    ...
    el.innerHTML = rows.length ? rows.join('') : '<p class="muted">Set goals in the Goals tab to see progress here.</p>';
  }
  ...
  on('settings:changed', () => {
    // TDEE changes affect projections
    // Charts will refresh on dashboard open; keep this light.
  });
})();
