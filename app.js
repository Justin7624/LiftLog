...
function estimateBodyFatPercent(opts){
  ...
  let bf = null;
  if (sx === 'male' && waist && neck && hIn){
    // US Navy (male)
    if(waist > neck){
      bf = 86.010*log10(waist - neck) - 70.041*log10(hIn) + 36.76;
    }
  } else if (sx === 'female' && waist && hip && neck && hIn){
    // US Navy (female)
    if(waist + hip > neck){
  ...
  if (bf == null && waist && wLb){
    // YMCA (waist & weight)
    if (sx === 'male'){
      bf = (((waist * 4.15) - (wLb * 0.082) - 98.42) / wLb) * 100;
    } else {
      bf = (((waist * 4.15) - (wLb * 0.082) - 76.76) / wLb) * 100;
    }
  }
...
$('#estimateBF').addEventListener('click', ()=>{
  ...
  $('#bfBox').innerHTML = `Estimated BF%: <strong>${bf.toFixed(1)}%</strong> (uses Navy/ YMCA/ BMI based on available measurements).`;
...
$('#estimateWeight').addEventListener('click', ()=>{
  ...
  let curBF = latest.bodyFat;
  if(curBF == null){
    curBF = estimateBodyFatPercent({sex:settings.sex, age:settings.age, heightInches:hIn, weightLb:curWeight, waistIn:latest.waist, hipIn:latest.hip, neckIn:latest.neck});
  }
  ...
  const bfs = weeks.map(w => {
    ...
    return +(Math.max(3, Math.min(60, 100*fatMass/Math.max(totW,1)))).toFixed(1);
  });
  renderProjectionChart(weeks, weights, bfs);

  $('#projectionBox').innerHTML = `TDEE est: <strong>${Math.round(tdee)}</strong> kcal/day. At ${cals} kcal, projected <strong>${sign}</strong> ≈ ${Math.abs(lbPerWeek).toFixed(2)} lb/week.<br><span class="muted">Projection assumes ~${Math.round(fatFrac*100)}% of weight change is fat mass.</span>`;
});
